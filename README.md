...
**Compose:** `pv_board(a, b, ...)` lays finished charts out as one responsive page — no Shiny, no R Markdown; print it, embed it in a chunk, or `pv_save()` it as `.html`, `.png`, or `.pdf` ([live example](https://jastephan63.github.io/polyviz/board.html)). A board is plain HTML rather than one widget, so a screen reader gets its headings and then each panel's own generated description. And three time-series tools keep the statistics in R, on base `stats` alone: `pv_decompose()` splits a seasonal series into aligned trend/seasonal/remainder panels (`stl` or classical), `pv_forecast()` extends a single-series line chart with a fan of model-based intervals — Holt-Winters, a small ARIMA search, or the naive baseline, the fitted model always recorded in the payload — and `pv_changepoints()` marks sustained level shifts found by binary segmentation with a BIC stopping rule. Each states plainly what it assumes and what its intervals don't know.
