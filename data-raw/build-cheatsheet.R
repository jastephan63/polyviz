...
col3 <- tag$div(class = "col",
  ...
  section("Compose & statistics",
    ...
    def("w |> pv_forecast(h)",
        "a fan of model-based intervals: Holt-Winters, small ARIMA, naive"),
    def("w |> pv_changepoints()",
        "marks sustained level shifts \u2014 binary segmentation + BIC")))
...
col4 <- tag$div(class = "col",
  section("Onto paper",
    ...
    def('pv_deck(charts, "review.pptx")',
        "a list of charts to PowerPoint, one 2\u00d7 capture per slide")),
  ...
  section("Theme & locale",
    def("pv_set_theme(pv_theme_paper())",
        list("the print-first paper theme; ", fn("pv_reset_theme()"),
             " undoes it")),
    def('pv_locale("de-CH")',
        list("numbers and dates the Swiss way (10\u2019000, M\u00e4rz); ",
             "fr/it/en-CH too; ", fn("NULL"), " resets"))),
  section("Bundled data",
    tag$p(class = "note",
      "17 datasets on board: ", fn("pv_sales"), ", ",
      fn("pv_network"), " and ", fn("pv_flows"),
      " are simulated; the rest is real Swiss open data \u2014 cities, ",
      "commuters, fiscal equalization, elections, weather, ",
      "electricity, tourism \u2014 plus the maps\u2019 boundary layers.")))
